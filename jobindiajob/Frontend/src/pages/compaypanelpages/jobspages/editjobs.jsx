...
const Editjobs = () => {
  ...
  useEffect(() => {
    setCompanyName(location.state.val.companyName);
    setPosition(location.state.val.position);
    setJobType(location.state.val.employmentType);
    setExperience(location.state.val.experienceLevel);
    setCloseDate(location.state.val.closedDate);
    setLastDateToApply(location.state.val.lastDate);
    setCity(location.state.val.city);
    setState(location.state.val.state);
    setDescription(location.state.val.description);
    setSkillsRequired(location.state.val.skillsRequired || [""]);
    setStatus(location.state.val.status);
    setSalaryFrom(location.state.val.salaryFrom);
    setSalaryTo(location.state.val.salaryTo);
  }, [location.state]);
  useEffect(() => {
  ...
  const addSkillField = () => {
    setSkillsRequired([...skillsRequired, ""]); // This is correct
  };
  ...
  const handleSkillChange = (index, event) => {
    ...
    newSkills[index] = event.target.value; // Update this line
    setSkillsRequired(newSkills);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${apiurl}/company/updatejob?id=${location?.state?.val._id}`, jobData, {
        withCredentials: true,
      });
      if (response.data.success) {
        console.log("Job updateed successfully:", response.data);
        navigate("/showjobs");
      } else {
    ...
    <div className="d-flex">
      ...
      <div className="flex-grow-1">
        ...
        <main className="p-4" style={{ backgroundColor: "#f5f5f5" }}>
          <h1 className="mb-4 fs-24 big-font-color ">Edit Job</h1>
